...
This WIP introduces new forward-compatibility requirements for implementations of the
devp2p Wire Protocol, the RLPx Discovery Protocol and the RLPx TCP Transport Protocol.
Clients which implement WIP-8 behave according to Postel's Law:
...
encoding for the encrypted key establishment handshake packets. If an WIP-8 style RLPx
...
(Auth₂) WIP-8 format with version 4 and no additional list elements (sent from A to B):
```text
...
(Auth₃) WIP-8 format with version 56 and 3 additional list elements (sent from A to B):
```text
...
(Ack₂) WIP-8 format with version 4 and no additional list elements (sent from B to A):
```text
...
(Ack₃) WIP-8 format with version 57 and 3 additional list elements (sent from B to A):
```text
