...
## Simple Summary
A standard way of representing various transactions, especially payment requests in Ethers and WRC #20 tokens as URLs.
...
This specification supersedes WRC #67, which is a URL format for representing arbitrary transactions in a low-level fashion. This WRC focuses specifically on the important special case of payment requests, while allowing for other, ABI-specified transactions.
...
Note that a `number` can be expressed in *scientific notation*, with a multiplier of a power of 10. The use of this notation is strongly encouraged when expressing monetary value in Ethers or WRC #20 tokens in atomic units (e. g. Wei, in case of Ether).
...
For the syntax of ENS_NAME, please consult WRC #137 defining Ethereum Name Service.
...
Requesting payments in WRC #20 tokens involves a request to call the `transfer` function of the token contract with an `address` and a `uint256` typed parameter, containing the *beneficiary address* and the *amount in atomic units*, respectively. For example,
...
Future upgrades that are partially or fully incompatible with this proposal must use a prefix other than `pay-` that is separated by a dash (`-`) character from whatever follows it, as specified by WRC #831.
